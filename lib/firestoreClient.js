...
class FirestoreClient {
  ...
  async createDocument(collection, docId, data) {
    ...
    try {
      ...
      throw new Error(
        `Firestore membalas status ${res.status} tapi body-nya bukan JSON valid saat membuat ${collection}/${docId}: ${text}`
      );
    }
    // Firestore REST API selalu balas objek dokumen berisi `name` kalau BENAR
    // tersimpan (mis. "projects/{proj}/databases/(default)/documents/licenses/ABC1234").
    // Kalau field ini tidak ada, dokumennya TIDAK benar-benar tersimpan meski
    // HTTP status 200 — jangan anggap sukses.
    if (!json.name) {
      throw new Error(
        `Firestore tidak mengembalikan dokumen yang tersimpan untuk ${collection}/${docId} (response: ${text}). ` +
          `Kemungkinan besar serviceAccountKey.json yang dipakai menunjuk ke project Firebase yang salah.`
      );
    }
    // Sanity check tambahan: pastikan document path yang dikembalikan Firestore
    // benar-benar cocok dengan collection + docId yang diminta (bukan project lain).
    const expectedSuffix = `/${collection}/${docId}`;
    if (!json.name.endsWith(expectedSuffix)) {
      throw new Error(
        `Dokumen tersimpan di path tak terduga "${json.name}", diharapkan berakhiran "${expectedSuffix}".`
      );
    }
    return FirestoreClient.fromFirestoreFields(json.fields);
  }
  ...
  async queryByField(collection, fieldName, value, { limit = 20 } = {}) {
    ...
    if (!Array.isArray(results)) {
      throw new Error(
        `Gagal query ${collection} where ${fieldName}=${value}: respons tak terduga (bukan array): ${JSON.stringify(results)}`
      );
    }
    return results
      .filter((r) => r.document)
