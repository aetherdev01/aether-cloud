...
async function createLicense(firestore, { token, days, note }) {
  ...
  // Verifikasi read-back: baca ulang dokumen yang baru saja dibuat langsung
  // dari Firestore (bukan cuma percaya response create). Ini lapis pertahanan
  // kedua kalau-kalau createDocument entah bagaimana "sukses" tapi menulis ke
  // tempat yang salah (mis. serviceAccountKey.json project yang keliru) —
  // daripada bot bilang berhasil padahal lisensinya tidak akan pernah
  // ditemukan aplikasi Android.
  const verified = await getLicense(firestore, token);
  if (!verified) {
    throw new Error(
      `Lisensi ${token} dilaporkan berhasil dibuat tapi TIDAK ditemukan saat dibaca ulang. ` +
        `Kemungkinan besar serviceAccountKey.json menunjuk ke project Firebase yang salah — ` +
        `cek EXPECTED_PROJECT_ID di .env.`
    );
  }
  return verified;
}
